...
module XeroRuby
  class ApiClient
    ...
    def authorization_url
      "#{@config.login_url}?response_type=code&client_id=#{@client_id}&redirect_uri=#{@redirect_uri}&scope=#{@scopes}"
    end
    ...
    def token_request(data)
      response = Faraday.post(@config.token_url) { |req|
    ...
    # Connection heplers
    def connections
      response = Faraday.get("https://api.xero.com/connections") { |req|
    ...
    def call_api(http_method, path, opts = {})
      ...
      connection = Faraday.new(url: config.base_url, ssl: ssl_options) { |conn|
      ...
      begin
        response = connection.public_send(http_method.to_sym.downcase) { |req|
    ...
    def api_set_prefix(return_type)
      ...
      Find.find("#{File.dirname(__FILE__)}/models/") do |path|
        ...
        matching_name = model_path.last.gsub(".rb", "").delete("_")
